...
// Google redirige acá (es la URL pública registrada en Google Cloud). Reenviamos
// el code/state server-to-server a tools-bridge, que hace el canje real y
// persiste el refresh token en su .env; acá solo mostramos el resultado.
export async function GET(req: NextRequest) {
  const bridgeUrl = process.env.TOOLS_BRIDGE_URL || "http://tools-bridge:4100";
  const qs = req.nextUrl.search;
  try {
    const res = await fetch(`${bridgeUrl}/api/auth/google/callback${qs}`, {
      cache: "no-store",
